...
/// Every method is failure-tolerant ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â returns null/empty/false on error
...
class ApiService {
  ...
  // Sprint-7: Incident reports, evidence vault, and case tracker
  Future<Map<String, dynamic>> getSafetyToolsDashboard() async {
    final res = await _safeGet(ApiConfig.safetyToolsDashboard);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<List<dynamic>> getSafetyIncidentReports() async {
    final res = await _safeGet(ApiConfig.safetyIncidentReports);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['reports', 'data']);
    return [];
  }

  Future<Map<String, dynamic>> createSafetyIncidentReport(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.safetyIncidentReports, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  }

  Future<List<dynamic>> getEvidenceVaultItems() async {
    final res = await _safeGet(ApiConfig.safetyEvidence);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['items', 'data']);
    return [];
  }

  Future<Map<String, dynamic>> createEvidenceVaultItem(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.safetyEvidence, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<List<dynamic>> getCaseTrackers() async {
    final res = await _safeGet(ApiConfig.safetyCases);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['cases', 'data']);
    return [];
  }

  Future<Map<String, dynamic>> createCaseTracker(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.safetyCases, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<Map<String, dynamic>> getEmergencyToolsDashboard() async {
    final res = await _safeGet(ApiConfig.emergencyToolsDashboard);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<Map<String, dynamic>> createDecoyCall(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.emergencyDecoyCall, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<Map<String, dynamic>> logStealthEvent(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.emergencyStealthEvent, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<Map<String, dynamic>> createWitnessReport(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.emergencyWitnessReport, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<Map<String, dynamic>> createAllyAlert(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.emergencyAllyAlert, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    body['message'] ??= _firstValidationError(body);
    return body;
  ...
  Future<List<dynamic>> getEmergencyHelplines() async {
    final res = await _safeGet(ApiConfig.emergencyHelplines);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['helplines', 'data']);
    return [];
  }}