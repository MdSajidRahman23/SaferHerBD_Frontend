...
/// Every method is failure-tolerant ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â returns null/empty/false on error
...
class ApiService {
  ...
  Future<Map<String, dynamic>> getAdminOverview() async {
    final res = await _safeGet(ApiConfig.adminOverview);
    final body = _parseJson(res) ?? <String, dynamic>{};
    body['statusCode'] = res.statusCode;
    if (res.statusCode == 200 && body['success'] != false) {
      body['success'] = true;
      return body;
    ...
    return {
      ...
      'message': body['message']?.toString() ??
          (res.statusCode == 403 ? 'Admin access required. Set ADMIN_PHONES or users.is_admin.' : 'Admin dashboard unavailable.'),
    };
  }

  Future<List<dynamic>> getAdminActiveSos() async {
    final res = await _safeGet(ApiConfig.adminActiveSos);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['alerts', 'data']);
    return [];
  }

  Future<List<dynamic>> getAdminModerationQueue() async {
    final res = await _safeGet(ApiConfig.adminModerationQueue);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['posts', 'data']);
    return [];
  }
  ...
  Future<Map<String, dynamic>> resolveAdminSos(String id) async {
    final res = await _safePost(ApiConfig.adminResolveSos(id), {
      'note': 'Resolved from SafeHerBD admin dashboard',
    });
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }
  Future<Map<String, dynamic>> approveAdminPost(String id) async {
    final res = await _safePost(ApiConfig.adminApprovePost(id), {});
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<Map<String, dynamic>> removeAdminPost(String id) async {
    final res = await _safePost(ApiConfig.adminRemovePost(id), {});
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }
  // Community Safety Hub: women-only forum, guardian links, sub-admin incidents
  Future<Map<String, dynamic>> getCommunitySafetyProfile() async {
    final res = await _safeGet(ApiConfig.communityProfile);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<Map<String, dynamic>> requestWomenForumVerification(String gender) async {
    final res = await _safePost(ApiConfig.womenForumRequestVerification, {'gender': gender});
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<List<dynamic>> getGuardianLinks() async {
    final res = await _safeGet(ApiConfig.guardianLinks);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['links', 'data']);
    return [];
  }

  Future<Map<String, dynamic>> createGuardianLink(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.guardianLinks, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }

  Future<List<dynamic>> getGuardianWards() async {
    final res = await _safeGet(ApiConfig.guardianWards);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['wards', 'data']);
    return [];
  }

  Future<List<dynamic>> getSubAdminIncidents() async {
    final res = await _safeGet(ApiConfig.subAdminIncidents);
    final body = _parseJson(res) ?? {};
    if (res.statusCode == 200) return _extractList(body, ['incidents', 'data']);
    return [];
  }

  Future<Map<String, dynamic>> submitSubAdminIncident(Map<String, dynamic> payload) async {
    final res = await _safePost(ApiConfig.subAdminIncidents, payload);
    final body = _parseJson(res) ?? {};
    body['statusCode'] = res.statusCode;
    return body;
  }
